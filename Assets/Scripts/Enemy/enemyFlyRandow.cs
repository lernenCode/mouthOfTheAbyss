...
public class enemyFlyRandow : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask whatIsPlataform;
    [SerializeField] private LayerMask whatIsCarry;
    ...
    {
        isGroud = Physics2D.OverlapCircle(groundCheck.transform.position, groundCheckRange, whatIsGround | whatIsCarry | whatIsPlataform);  // Verifica colisão do chao   
        isWall = Physics2D.OverlapCircle(wallCheck.transform.position, wallCheckRange, whatIsGround | whatIsCarry | whatIsPlataform);       // Verifica colisão da parede  
        isCeil = Physics2D.OverlapCircle(ceilingCheck.transform.position, ceilingCheckRange, whatIsGround | whatIsCarry | whatIsPlataform); // Verifica colisão do teto    
    }
